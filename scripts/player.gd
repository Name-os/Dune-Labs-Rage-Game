...
var jump_duration := 3 #how many frames the pushoff animation shows
...
func _ready():
	shroom_jump_radius *= shroom_jump_radius #squared so we can use distance_squared_to, faster than square rooting
...
func climb(im):
	...
	if im["jump"]:
		...
		jump_frames = jump_duration #starts the pushoff animation timer
		stamina -= stamina_cost_wall_jump
		stamina = cap(stamina, 0, null)
		var ratio = cap(stamina / stamina_cost_wall_jump, null, 1.0)
...
func get_input():
	...
	crouching = false if not is_on_floor() else crouching
	climbing = false if wall_jump_cooldown < 1 else climbing
